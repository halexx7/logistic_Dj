...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        # F objects means that all calculation will be in database with SQL query
        action_1__price = When(action_1__condition, then=F("service__price") * F("quantity") * action_1__discount)
        action_2__price = When(action_2__condition, then=F("service__price") * F("quantity") * -action_2__discount)
        action_expired__price = When(
            action_expired__condition, then=F("service__price") * F("quantity") * action_expired__discount
        )
        ...
        for orderitem in test_orders:
            print(
                f"{orderitem.action_order:2}: заказ №{orderitem.pk:3}: {orderitem.service.name:15}: скидка {abs(orderitem.total_discount):9.2f} руб. | {orderitem.order.updated - orderitem.order.created}"
            )
